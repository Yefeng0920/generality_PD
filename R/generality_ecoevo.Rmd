...
# how many significant effects with PI spaning 0
...
h_stats <- readRDS(here("Data", "h_stats.Rds"))
...
h_stats_rob <- readRDS(here("Data", "h_stats_rob.Rds"))
```
...
```{r}
# how many CIs did not span 0
which((h_stats_all$ci.lb*h_stats_all$ci.ub)>0) %>% length() # 321

# how many PIs did not span 0
which((h_stats_all$pi.lb*h_stats_all$pi.ub)>0) %>% length() # 21

# how many significant effects with PI spaning 0
which((h_stats_all$ci.lb*h_stats_all$ci.ub)>0 & (h_stats_all$pi.lb*h_stats_all$pi.ub)<0) %>% length() # 300
```
...
scenario1 <- h_stats_all %>% filter(I2_t > 50 & CV_t < 1)

#  scenario2 - medium to large I2 but small V bar
scenario2 <- h_stats_all %>% filter(I2_t > 50 & v_bar_to_beta0 < 1)
...
# extract variance
sigma_stats <- data.frame(beta = sapply(c(model_all,model_all_rob), function(x) x$beta[1]),
                          beta.se = sapply(c(model_all,model_all_rob), function(x) x$se[1]),
                          beta.p = sapply(c(model_all,model_all_rob), function(x) x$pval[1]),
                          sigma2_t = sapply(c(model_all,model_all_rob), function(x) sum(x$sigma2)),
                          sigma2_b = sapply(c(model_all,model_all_rob), function(x) x$sigma2[1]),
                          sigma2_w = sapply(c(model_all,model_all_rob), function(x) x$sigma2[2]),
                          QEp = sapply(c(model_all,model_all_rob), function(x) x$QEp))

# combine
PD_data <- cbind(sigma_stats, PD_data)
...
```{r}
# PI
model_all[[4]]$beta[1] + qt(1 - 0.05/2, df = model_all[[4]]$ddf[[1]]) * sqrt(sum(model_all[[4]]$sigma2) + model_all[[4]]$se^2)
```
